...
from module.security import BlockWord, hash
...
def CodeGet():
    if request.method == "GET":
        ...
        DB_RES = DB.SELECT_ONE(f"SELECT * FROM kiosk.get_code WHERE get_true = 0")
        print(DB_RES)
        DB_COM = DB.UPDATE(f"UPDATE kiosk.get_code SET get_true = 1 WHERE Code = '{DB_RES['Code']}'")
        if DB_COM != True:
